...
from ansible_collections.tosit.tdp.plugins.module_utils.kerberos import (
    klist_kt,
    try_kinit,
)
from ansible_collections.tosit.tdp.plugins.module_utils.kerberos_admin import (
    kerberos_admin_spec,
    kadmin,
)
...
def main():
    ...
    try:
        ...
        # Either the keytab does not exist or it is not valid, so it must be generated only if there are new principals.
        results["changed"] = True
        if not module.check_mode:
            for principal in principals:
                if not os.path.exists(keytab_path) or principal not in klist_kt(
                    module, keytab_path
                ):
                    rc, stdout, stderr = kadmin(
                        module, ["-q", "ktadd -k {} {}".format(keytab_path, principal)]
                    )
                    # rc is 0 when the principal does not exist...
                    if "Principal" in stderr and "does not exist" in stderr:
                        raise RuntimeError(
                            "Failed to generate keytab for principal '{}': {}".format(
                                principal, stderr
                            )
                        )
            ...
            if not try_kinit(module, kinit_bin, kdestroy_bin, principals, keytab_path):
                raise RuntimeError(
                    "Keytab '{}' generated for principal '{}' is not working".format(
                        keytab_path, principals
                    )
                )
